    ...
    while retries_left > 0:
        if b64_file_size < ZIP_UPLOAD_LIMIT_SIZE:
            ...
            try:
                ...
                if isinstance(
                    exc, lambda_client.exceptions.InvalidParameterValueException
                ):
                    current_delay = 2 ** (max_retries - retries_left) * base_delay
                    logging.info(
                        f"Role defined could not yet be assumed by Lambda, retrying upload after delay of {current_delay}ms."
                    )
                    time.sleep(current_delay / 1000.0)
                    retries_left -= 1
                else:
                    logging.error("Failed with: {}".format(type(exc)))
                    logging.error("Details: {}".format(str(exc)[:2048]))
                    raise exc
        else:
            ...
            s3_client.upload_file(
                lambda_zip_file, s3_bucket, s3_key_obj, Callback=callback
            )
            ...
            try:
                ...
                if isinstance(
                    ...
                    current_delay = 2 ** (max_retries - retries_left) * base_delay
                    logging.info(
                        f"Role defined could not yet be assumed by Lambda, retrying upload to/from S3 after delay of {current_delay}ms."
                    )
                    time.sleep(current_delay / 1000.0)
                    retries_left -= 1
                    continue
